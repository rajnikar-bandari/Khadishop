...
?>
    ...
    <div id="poststuff">
		...
		<?php if ( $options['support_shortcode_products_on_sale'] OR $options['support_shortcode_products_bogo'] ): ?>
		...
		<?php if ( $options['support_shortcode_products_on_sale'] ): ?>
         ...
         <?php endif; ?>
        <?php if ($options['support_shortcode_products_bogo']): ?>
        ...
        <?php if(isset($_GET['product']) && isset($_GET['action_rules'])): ?>
                <div>
                    <span class="tag-show-rules-for-product"><?php printf(__('Only rules for product "%s" are shown', 'advanced-dynamic-pricing-for-woocommerce'), WDP_Helpers::get_product_title($_GET['product'])); ?></span>
                </div>
         <?php endif; ?>    
        <div style="clear: both;">
