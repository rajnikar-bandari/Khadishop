...
//    	return self::get_test_rules();
	    ...
	    foreach ( $rows as &$row ) {
		    foreach ( $row['conditions'] as &$condition_data ) {
			    if ( $condition_data['type'] === 'shipping_state' ) {
				    $comparison_value = $condition_data['options'][1];
				    ...
				    if ( $changed ) {
					    $condition_data['options'][1] = $new_comparison_value;
				    }
			    }
		    }
	    }
