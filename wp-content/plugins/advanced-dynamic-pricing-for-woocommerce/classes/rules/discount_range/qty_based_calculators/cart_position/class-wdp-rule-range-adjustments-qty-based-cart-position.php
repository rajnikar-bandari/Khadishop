...
class WDP_Rule_Range_Adjustments_Qty_Based_Cart_Position implements WDP_Rule_Range_Adjustments_Qty_Based {
	private $key = 'cart_position';
	private $rule_id;
	...
	public static function get_label() {
		return __( 'Qty based on cart position', 'advanced-dynamic-pricing-for-woocommerce' );
	}
	...
	public function process( $bulk_calc, $cart, $set_collection ) {
		...
		foreach ( $cart_items_collection->get_items() as $item ) {
			...
			// $product = $item_data['data']; //prop 'key' is cart position key maybe
			// /**
			...
			if ( ! isset( $item_key_hash_mapping[ $item_key ] ) ) {
				$item_key_hash_mapping[ $item_key ] = array();
			}
			$item_key_hash_mapping[ $item_key ][] = $hash;
		}

		foreach ( $item_key_hash_mapping as $item_key => $hashes ) {
			...
			foreach ( $bulk_calc->calculate_items_discounts( $range_calculation_items ) as $calculate_items_discount ) {
				...
				$price = $calculator->calculate_price_for_item( $cart_items_collection->get_item_by_hash( $calculate_items_discount->get_hash() ) );
				$cart_items_collection->set_price_for_item( $calculate_items_discount->get_hash(), $price, $calculate_items_discount->get_qty() );
			}
		}
